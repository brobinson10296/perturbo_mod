...
subroutine calc_elph_mat_wann(ep)
   use kinds, only: dp
   use ions_base, only: nat, tau
   use cell_base, only: at, bg, omega, tpiba
   use input_param, only: num_wann, kdim, polar_alpha, prefix, thickness_2d
   use lattice_data, only: numq, xq_tot, qdim, zstar, epsil
   use electronic_data, only: xk_tot, nkstot, wannier_center_cryst
   ...
   ! initialize
   kqfac = 1.0E0_dp / real(nkstot*numq, dp)
   ep%lpol = merge(.true., .false., sum(abs(zstar)) > 1.0E-1_dp)
   ...
   call set_ws_cell_eph &
      (ep, kdim, qdim, nat, num_wann, at, ctau, wannier_center_cryst, .false.)
...
!$omp end parallel do
   ...
   do ia = 1, nat
      !workspace
      allocate( epmat_loc(numb, numb, nkstot, nq_loc, 3) )
      
      !read and distribute data
      if(ionode) then
         allocate( epmat(numb, numb, nkstot, numq) )
      else
         allocate( epmat(1,1,1,1) )
      endif
      !load epmat data
      do ic = 1, 3
         if(ionode) call read_ephmat_part(elph_fid, ia, ic, numb, nkstot, numq, epmat)
      ...
      allocate( ep_tmp(3, nkstot, nq_loc, num_wann, num_wann) )
      ...
      do iw = 1, num_wann
         ...
         do i = 1, nq_loc
            iq = iq_st + i - 1
            call eph_bloch2wann(xq_tot(:,iq), xk_tot, nkstot, ep_tmp(:,:,i,iw,jw), &
...
subroutine open_elph_file(file_id, wannier_gauge, nb)
   ...
   !sanity check
   if( nkstot .ne. ndim(3) .or. numq .ne. ndim(4) ) &
...
subroutine rearrange_ephmat(nb, iq_st, nq_loc, wannier_gauge, epmat_in, epmat_out)
   ...
   use electronic_data, only: xk_tot, nkstot, ktable, kpt2num, rot_wan, num_wann
   implicit none
   integer, intent(in) :: nb, iq_st, nq_loc
   logical, intent(in) :: wannier_gauge
   complex(dp), intent(in) :: epmat_in(nb, nb, nkstot, nq_loc, 3)
   complex(dp), intent(out) :: epmat_out(3, nkstot, nq_loc, num_wann, num_wann)
   !local
   real(dp) :: xkqg(3)
...
!$omp do schedule(guided)
      do idx = 0, nkstot*nq_loc*3 - 1
         !idx = (ic-1)*nkstot*nq_loc + (iq-1)*nkstot + (ik-1)
         ik = mod(idx, nkstot) + 1
         iq = mod(idx/nkstot, nq_loc) + 1
         ic = idx / (nkstot*nq_loc) + 1
