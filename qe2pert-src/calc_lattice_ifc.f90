...
subroutine calc_lattice_ifc(ph, qdim, nat, at, ctau, numq, xq_tot, dynmat)
   use kinds, only: dp
   ...
   use lattice_data, only: zstar, count_q2m1, count_q2m2, count_q2m3
   ...
   call set_ws_cell_ph(ph, qdim, nat, at, ctau)
   !init
   do m = 1, nelem
   ...
   if (asr /= 'no' ) then 
      ...
      frc=real(phid)
      !
      CALL set_asr (asr, qdim(1), qdim(2), qdim(3), frc, zstar, nat, ibrav, ctau)
      !
      !copy frc to phid
      phid=frc
      !
