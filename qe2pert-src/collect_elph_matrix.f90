...
subroutine collect_elph_matrix()
   ...
   use electronic_data, only: nkstot, nb_sub
   ...
   if(ionode) then
      ...
      !
      allocate( epmat(nb_sub, nb_sub, nkstot, numq) )
   else
      allocate( epmat(1,1,1,1) )
   endif
   !allocate workspace
   allocate( epmat_loc(nb_sub, nb_sub, nkstot, nq_loc) )
   !
   ...
   do j = 1, 3
      ...
      !
      !output to hdf5 file
      if(ionode) call write_ephmat_part(file_id, ia, j, nb_sub, nkstot, numq, epmat)
      !call mp_barrier(inter_pool_comm)
   enddo; enddo
   !release space
