...
subroutine ph_dvqpsi_us_local &
  ...
  use electronic_data, only: ngk_tot, igk_k_tot, evc_sub, nb_sub
  !
...
!!$omp end master
  ...
  if (nlcc_any.and.addnlcc) then
     ...
     if (.not.lsda) then
        ...
        !drop support for lsda
        call errore('ph_dvqpsi_us_local','no support for lsda',1)
        !TODO: currently not support lsda
        !is=isk(ikk)
        !do ir=1,dfftp%nnr
