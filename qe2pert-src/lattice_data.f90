...
module lattice_data
   ...
   ! dielectric tensor
   real(dp), allocatable, save :: zstar(:,:,:)
   ...
   DO ifile=1,nfile
      ...
      IF (ifile == 1) THEN
         ...
         if(lrigid .AND. (asr.NE.'no')) then
            CALL set_zasr ( asr, nr1,nr2,nr3, nat, ibrav_, tau, zstar)
         endif
      END IF
      ...
      DO nq = 1,nqs
         ...
         IF(nc(m(1),m(2),m(3)).EQ.0) THEN
            ...
            loto_2d = merge(.true., .false., (sum(abs(zstar)) > 1.0E-1_dp).and.system_2d)

            IF (lrigid .and. lpolar) THEN
               CALL rgd_blk (nr1,nr2,nr3,nat,phiq(1,1,1,1,nq),q(1,nq), &
                  tau,epsil,zstar,bg_,omega,celldm_(1), loto_2d,-1.d0) ! 2D added celldm_ and flag
            END IF
