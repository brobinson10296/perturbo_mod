...
module boltz_dynamics_mod
   ...
   public :: restart_dist, init_dist_lorentz, init_dist_gaussian, init_dist_fermi, &
      output_dist, read_dist, calc_carrier_population, output_carrier_population, &
      calc_mean_velocity, init_dist_gaussian_plus_fermi !BR ADDED
contains
...
subroutine init_dist_gaussian(kgrid, dist, e0, smear, lhole, gauss_height) !BR ADDED
   implicit none
   type(grid), intent(in) :: kgrid
   real(dp), intent(in) :: e0, smear, gauss_height
   real(dp), intent(out) :: dist(kgrid%numb, kgrid%nk)
   !
   logical,  intent(in), optional :: lhole
   !local variables
   ...
   do ik = 1, kgrid%nk
      do ib = 1, kgrid%numb
         enk = (kgrid%enk(ib, ik) - e0) / smear
         dist(ib, ik) = gauss_height*exp( -min(enk*enk, 200.0_dp) ) !BR ADDED
      enddo
   enddo
   !in case of hole carrier
   if( present(lhole) ) then
...
!BR ADDED
subroutine init_dist_gaussian_plus_fermi(kgrid, dist, e0, smear, lhole, E_fermi, gauss_height) !BR ADDED
   use pert_utils, only: fermi
   implicit none
   type(grid), intent(in) :: kgrid
   real(dp), intent(in) :: e0, smear, E_fermi, gauss_height !BR ADDED
   real(dp), intent(out) :: dist(kgrid%numb, kgrid%nk)
   !
   logical,  intent(in), optional :: lhole
   !local variables
   integer :: ik, ib
   real(dp) :: enk, enk_gaussian, enk_fermi
   print*, E_fermi*ryd2ev !BR added
   do ik = 1, kgrid%nk
      do ib = 1, kgrid%numb
        !need to change enk to enk_gauss and enk_fermi
        !gaussian
         enk_gaussian = (kgrid%enk(ib, ik) - e0) / smear
         !dist(ib, ik) = 0.1_dp*exp( -min(enk_gauss*enk_gauss, 200.0_dp) )
         !fermi
         enk_fermi = kgrid%enk(ib, ik) - E_fermi
         !add gaussian and fermi distribution
         dist(ib, ik) = gauss_height*exp( -min(enk_gaussian*enk_gaussian, 200.0_dp) )+ fermi(smear, enk_fermi)
      enddo
   enddo
   !in case of hole carrier
   if( present(lhole) ) then
...
end subroutine init_dist_gaussian_plus_fermi
!BR ADDED
