...
subroutine carrier_dynamics_run()
   ...
   subroutine cdyna_setup(filename, kgrid, fid, gid, dist)
      ...
      use boltz_dynamics_mod, only: output_dist, restart_dist, &
         init_dist_fermi, init_dist_lorentz, init_dist_gaussian,init_dist_gaussian_plus_fermi !BR added
      !use pert_param, only: boltz_nstep, output_nstep, time_step, &
      !    boltz_init_dist, boltz_init_e0, boltz_init_smear, hole, E_fermi !BR added
      use pert_param, only: boltz_nstep, output_nstep, time_step, &
         boltz_init_dist, boltz_init_e0, boltz_init_smear, hole, E_fermi, gauss_height !BR added
      ...
      if(trim(boltz_init_dist) .eq. 'restart') then
         ...
         case('gaussian')
            call init_dist_gaussian(kgrid, dist, boltz_init_e0, boltz_init_smear, hole, gauss_height)
         case('gaussian_plus_fermi') !BR added
            call init_dist_gaussian_plus_fermi(kgrid, dist, boltz_init_e0, boltz_init_smear, hole, E_fermi, gauss_height) !BR added
         case default
